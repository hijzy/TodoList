...
function Workspace() {
	...
	async function uploadFiles(files: FileList | File[]) {
		...
		setUploadProgress(0);
		setUploadStatusText(Array.from(files).length === 1 ? `Uploading ${Array.from(files)[0].name}` : `Uploading ${Array.from(files).length} files`);
		try {
			...
			setUploadStatusText(error instanceof Error ? error.message : 'Upload failed');
	...
	function handleDropFiles(event: ReactDragEvent<HTMLDivElement>) {
		event.preventDefault();
	...
	useEffect(() => {
		...
		fetch('/tasks.json').then(response => {
			...
			.catch(() => {
				...
				setNoteContent(localStorage.getItem(CLOUD_CACHE_NOTE_KEY) ?? DEFAULT_NOTE_CONTENT);
	...
	return (
		<div className={`main ${isDropping ? 'is-dropping' : ''}`}>
				<div className="workspace-shell">
					...
					<div className="workspace-grid">
						...
						<div className="todo-scroll-area">
							...
							<h3 className="group-title">Tasks</h3>
							<ul
								ref={element => {
					...
					<section className="panel file-panel">
						...
						<div className="drop-zone" onClick={() => fileInputRef.current?.click()} onDragOver={event => event.preventDefault()} onDrop={handleDropFiles}>
							<div className="drop-zone-plus" aria-hidden="true">+</div>
							<p>{isUploadingFiles ? uploadStatusText || 'Uploading...' : 'Drop files here to upload'}</p>
							<span>{isUploadingFiles ? `${Math.round(uploadProgress * 100)}% complete` : 'or click to browse files'}</span>
							{isUploadingFiles && (
							...
							{stashedFiles.map(file => {
								...
								<li key={file.id} className="file-item">
									...
									<div className="file-actions">
										...
										<button type="button" className="danger-button" onClick={() => void handleDeleteFile(file.id)}>
											Delete
										</button>
									</div>
								</li>
